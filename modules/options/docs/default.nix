{
  ...
  #TODO: Kernal option seems to not to well here not sure why but we remove it for the options build.
  configMD = let
    # Function to recursively remove a specified path from an attribute set
    removePath = path: set: let
    ...
    in
      ...
      then
        if tail == []
        then builtins.removeAttrs set [head]
        else set // {${head} = removePath tail set.${head};}
      else set;

    # Updated modules with 'system.boot.kernel' removed
    modulesWithoutKernel = removePath ["system" "boot" "kernel"] options.modules;
  in
    (pkgs.nixosOptionsDoc {
      options = modulesWithoutKernel;
